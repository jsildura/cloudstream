...
const StudioPage = () => {
    ...
    const getFilteredMovies = () => {
        ...
        if (filters['vote_average.gte']) {
            filtered = filtered.filter(movie => movie.vote_average >= filters['vote_average.gte']);
        }
    ...
    const getFilteredTVShows = () => {
        ...
        if (filters['vote_average.gte']) {
            filtered = filtered.filter(show => show.vote_average >= filters['vote_average.gte']);
        }
    ...
    // Initial fetch of studio content
    useEffect(() => {
        const fetchInitialContent = async () => {
            try {
                setLoading(true);

                // Fetch first 5 pages for each type
                const moviePromises = [];
                const tvPromises = [];

                for (let page = 1; page <= 5; page++) {
                    moviePromises.push(
                        fetch(`/api/discover/movie?with_companies=${studioId}&sort_by=popularity.desc&page=${page}`)
                    );
                    tvPromises.push(
                        fetch(`/api/discover/tv?with_companies=${studioId}&sort_by=popularity.desc&page=${page}`)
                    );
                }
        ...
        <div className="studio-page">
            ...
            <div className="studio-page-header" style={{ '--studio-color': studioInfo.color, '--studio-color-rgb': studioInfo.colorRgb }}>
                ...
                <p className="studio-page-subtitle">
                    Movies and TV shows from {studioInfo.name}
                </p>
            </div>
            ...
            <section className="studio-section">
                <div className="studio-section-header">
                    <div className="studio-section-accent" style={{ background: studioInfo.color }}></div>
                    <h2 className="studio-section-title">Movies</h2>
                    <span className="studio-section-count">{filteredMovies.length} titles</span>
                    <button
                ...
                {moviesPage < moviesTotalPages && (
                    <div className="load-more-container">
                        <button
                            ...
                            style={{ borderColor: studioInfo.color, background: `${studioInfo.color}33` }}
                        >
                            {loadingMoreMovies ? (
                                <>
            ...
            {filteredTVShows.length > 0 && (
                <section className="studio-section">
                    <div className="studio-section-header">
                        <div className="studio-section-accent" style={{ background: studioInfo.color }}></div>
                        <h2 className="studio-section-title">TV Shows</h2>
                        <span className="studio-section-count">{filteredTVShows.length} titles</span>
                    </div>
                    <div className="studio-grid">
