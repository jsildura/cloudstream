...
const Peacock = () => {
        ...
        const fetchInitialContent = async () => {
            try {
                ...
                for (let page = 1; page <= 5; page++) {
                    moviePromises.push(fetch(`/api/discover/movie?with_watch_providers=386&watch_region=US&with_watch_monetization_types=flatrate&sort_by=popularity.desc&page=${page}`));
                    tvPromises.push(fetch(`/api/discover/tv?with_networks=3353&sort_by=popularity.desc&page=${page}`));
                }
            ...
            } catch (err) { console.error('Error fetching Peacock content:', err); } finally { setLoading(false); }
        };
        fetchInitialContent();
    }, []);
    ...
    const loadMoreMovies = async () => {
        ...
        try {
            const nextPages = [];
            for (let page = moviesPage + 1; page <= Math.min(moviesPage + 3, moviesTotalPages); page++) nextPages.push(fetch(`/api/discover/movie?with_watch_providers=386&watch_region=US&with_watch_monetization_types=flatrate&sort_by=popularity.desc&page=${page}`));
    ...
    const loadMoreTV = async () => {
        ...
        try {
            const nextPages = [];
            for (let page = tvPage + 1; page <= Math.min(tvPage + 3, tvTotalPages); page++) nextPages.push(fetch(`/api/discover/tv?with_networks=3353&sort_by=popularity.desc&page=${page}`));
            const data = await Promise.all((await Promise.all(nextPages)).map(res => res.json()));
    ...
    if (loading) return (<div className="peacock-page"><div className="peacock-page-header"><Link to="/" className="back-button"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m12 19-7-7 7-7" /><path d="M19 12H5" /></svg>Back</Link><div className="peacock-page-title-section"><img src="/provider/peacock_logo.png" alt="Peacock" className="peacock-page-logo" /></div></div><div className="peacock-page-loading"><div className="loading-spinner" /><p>Loading Peacock content...</p></div></div>);
    ...
    return (
        <div className="peacock-page">
            <div className="peacock-page-header">
                ...
                <div className="peacock-page-title-section"><img src="/provider/peacock_logo.png" alt="Peacock" className="peacock-page-logo" /></div>
                <p className="peacock-page-subtitle">Peacock Original series and featured movies</p>
            </div>
            ...
            <section className="peacock-content-section">
                <div className="peacock-section-header">
                ...
                <span className="peacock-section-count">{currentContent.length} titles</span>
                <div className="peacock-grid">{currentContent.map(item => <MovieCard key={item.id} item={item} onClick={() => handleItemClick(item)} />)}</div>
                {currentPage < totalPages && <div className="load-more-container"><button className="load-more-btn peacock-load-more" onClick={loadMore} disabled={isLoadingMore}>{isLoadingMore ? <><div className="btn-spinner" />Loading...</> : <>Load More {activeMediaType === 'movie' ? 'Movies' : 'TV Shows'}<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m6 9 6 6 6-6" /></svg></>}</button></div>}
            </section>
