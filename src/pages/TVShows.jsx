...
const TVShows = () => {
  ...
  const handleItemClick = useCallback(async (item) => {
    closeNow();
    const type = 'tv';
    const genreNames = item.genre_ids?.map(id => tvGenres.get(id)).filter(Boolean) || [];
    ...
    setIsModalOpen(true);
  }, [closeNow, tvGenres, fetchCredits, fetchContentRating]);
  ...
  const handleApplyFilters = (pending) => {
    setFilters({
      ...
      first_air_date_year: pending.year || undefined,
  ...
  const activeFilterCount =
    ...
    (String(filters.first_air_date_year ?? '') !== PANEL_DEFAULTS.year ? 1 : 0) +
    ...
    <div className="tv-shows-page">
      ...
      <div className="content-rows">
        <DiscoverGrid
          ...
          error={feed.error}
          emptyMessage="No TV shows match these filters. Try clearing a few."
